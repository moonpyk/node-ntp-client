...
(function (exports) {
	...
	// some errors can happen before/after .send or cause .send to be impossible
	// Some errors will be given to the .send callback also. This is bad.
	// Keep a flag, therefore, to prevent multiple callbacks.
	// NOTE the error callback is not generalised, as the client has to
	...
	client.on('error',function(err) {
		...
		callback(err,null);
		fired = 1;
		// Watchdog timeout is still waiting. Will cause error if not cleared.
